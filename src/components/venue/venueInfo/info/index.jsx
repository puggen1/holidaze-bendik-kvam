...
const Info = ({name, description, maxGuests, price, owner, meta}) => {
  return (
    <InnerInfo>
       ...
       <Link className='host' style={{textDecoration:"none"}} to={`/user/${owner.name}`}> <Typography fontFamily="Roboto" fontWeight={400} fontSize="1.2rem">Host is {owner.name}</Typography><Avatar /></Link>
